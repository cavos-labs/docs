...
Cavos provides **gasless transactions by default** — users never need to hold ETH or STRK. When you need the wallet to pay its own gas, pass `{ gasless: false }` to `execute()`.

```tsx
// Gasless (default) — gas sponsored by Cavos Paymaster
await execute(calls);

// User-pays — wallet pays from its own STRK balance
