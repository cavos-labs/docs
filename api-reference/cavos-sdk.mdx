...
Send and verify a passwordless email OTP code. Account deployment and session registration start automatically after `verifyOtp()`.

```typescript
await sdk.sendOtp('user@example.com');
await sdk.verifyOtp('user@example.com', '123456');
```

Aliases are also available:

```typescript
await sdk.requestOTP('user@example.com');
await sdk.loginOTP('user@example.com', '123456');
```
...
Execute transactions on a Cartridge Slot (Katana). Always fee-free, no paymaster needed. Handles account deployment and session registration on the Slot automatically on the first call.
