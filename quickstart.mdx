...
**How it works:**
1. After login, SDK generates an session key
2. First transaction: Deploys account + registers session key + executes
3. Subsequent transactions: Signed with session key (fast & cheap)
...
sequenceDiagram
    ...
    Note over Chain: Verifies JWT RSA signature<br/>Registers session key
