...
description: "Social login, email/passwordless, magic link, and OTP authentication flows"
---
...
| **Email OTP** | Email | Passwordless code-based email login |
...
Send a six-digit email code, then verify it to complete login. This is a two-step flow for custom UIs.
...
function OtpForm() {
  const { sendOtp, verifyOtp, isAuthenticated, address } = useCavos();
  const [email, setEmail] = useState('');
  ...
  return sent ? (
    <form onSubmit={async (e) => { e.preventDefault(); await verifyOtp(email, code); }}>
      <input value={code} onChange={(e) => setCode(e.target.value)} placeholder="123456" />
      <button type="submit">Verify code</button>
    </form>
  ) : (
    <form onSubmit={async (e) => { e.preventDefault(); await sendOtp(email); setSent(true); }}>
...
The raw SDK also provides aliases:

```ts
await cavos.requestOTP('user@example.com');
await cavos.loginOTP('user@example.com', '123456');
```

### How Email OTP Works

1. `sendOtp(email)` creates a pre-auth session and sends the code
2. `verifyOtp(email, code)` verifies the code and completes authentication
3. Account deployment + session registration happen automatically
