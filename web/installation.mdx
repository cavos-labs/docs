...
If your app runs on a [Cartridge Slot](https://docs.cartridge.gg/slot) (a private, fee-free Katana chain forked from mainnet) or a local Katana node, add the `slot` key to your config. This unlocks `executeOnSlot()` — gasless transactions on the Slot chain with no paymaster needed.
